...
func change_screen(new_screen: Types.screens):
	if is_changing_screen:
		return
	
	is_changing_screen = true
	
	var new_screen_node : GameScreen = Constants.screen_scenes[new_screen].instatntiate()
	load_new_screen(new_screen_node, new_screen)
	
func load_new_screen(new_screen_node: GameScreen, new_screen: Types.screens):
